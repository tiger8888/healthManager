...
//    [_dataSourceCheck setObject:[NSNumber numberWithInt:checked]  atIndexedSubscript:checkbox.tag];
    ...
    [_dataSourceCheck replaceObjectAtIndex:checkbox.tag withObject:[NSNumber numberWithInt:checked]];
}
#pragma mark - 自定义方法
...
- (IBAction)clickSubmit:(id)sender {
    ...
    NSString *periodStr = [[_dataSourceCheck valueForKey:@"description"] componentsJoinedByString:@""];
    if ([periodStr isEqualToString:@"0000000"]) {
        ALERT(@"", @"请选择提醒周期。", @"确定");
        return;
    }
    if (self.medince) {
        //更新
        [self.medince setValue:self.medicineName.text forKey:@"name"];
        [self.medince setValue:periodStr forKey:@"period"];
        if ([[MedinceRecordManager sharedManager] updateOne:self.medince]) {
            ALERT(@"", @"修改药品成功。", @"确定");
            if (self.block) {
                self.block();
            }
        ...
        medinceRecord.period = periodStr;
...
- (void)showEditingData {
    if (self.medince) {
        self.medicineName.text = [self.medince valueForKey:@"name"];
        NSString *weekCheck = [self.medince valueForKey:@"period"];
        if (weekCheck.length!=7 || [weekCheck isEqualToString:@"1111111"]) {
            //默认值，不用进行格式转换
        }
        else {
            for (int i=0; i<weekCheck.length; i++) {
                if ( [weekCheck characterAtIndex:i] != '1') {
                    [_dataSourceCheck replaceObjectAtIndex:i withObject:@"0"];
//                    [_dataSourceCheck setObject:@"0" atIndexedSubscript:i];
                }
            }
        }
    }

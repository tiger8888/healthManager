...
- (void)CreateTable {
    NSString *sql_create = @"create table if not exists sessionmessage(id integer primary key, senderId integer, senderName text, content text, sendType integer, timeStamp text);";
...
- (void)insertOne:(SessionMessage *)msg {
    char *sql_insert = "insert or replace into sessionmessage(senderId, senderName, content, sendType, timeStamp) values(?,?,?,?,?)";
